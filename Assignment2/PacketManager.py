...
class Packet:
    ...
    def toBinaryString(self, data_size:int):
        ...
        for i in range(0, len(segmentData)):
            ...
            dataByte = '{0:08b}'.format(ord(character))   # convert each character to equivalent ASCII code
            data = data + dataByte
        packet = preamble + sfd + destAddress + sourceAddress + typeToBits + seqToBits + data
        packet = errordetect.crc(packet,crc)
        self.packet = packet
        return packet
    ...
    def build(cls,packet):
        ...
        for letter in asciiData:
            if letter== '00000000':
                continue
            text += chr(int(letter,2))  # getting the character from the ASCII code
    ...
    def hasError(self):
        return errordetect.detect_error_crc(self.packet, crc)

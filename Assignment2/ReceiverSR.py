...
class Receiver:
    ...
    def validSEQ(self, seq_no: int):
        if (self.front <= seq_no < self.end) or (self.end < self.front <= seq_no) or (seq_no < self.end < self.front):
            return True
        else:
            return False

    def sendAck(self):
        packet = PacketManager.Packet(self.senderAddress, self.receiverAddress, self.packetType['ack'], self.front,
    ...
    def sendNak(self):
        packet = PacketManager.Packet(self.senderAddress, self.receiverAddress, self.packetType['nak'], self.front,
                                      'No acknowledgement')
        self.connection.send(str.encode(packet.toBinaryString(22)))
